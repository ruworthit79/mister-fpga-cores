...
| 2 | **Integrated FPU** | on-die 68040 FPU | 🟡 | **Foundation + kernel dispatch + HW arithmetic all live and wired** (`fpu_040.vhd`): FP0-FP7 + FPCR/FPSR/FPIAR, FMOVE/FABS/FNEG/FTST, IEEE classification, `present=1`, `unimpl`→FPSP. The CPU **executes reg-reg F-line arithmetic in hardware** — the kernel decodes **FADD ($22) / FSUB ($28) / FMUL ($23) / FDIV ($20) / FSQRT ($04)** and drives the 80-bit extended datapath (NaN/Inf/Zero handling, normalization, digit-by-digit sqrt). The FPU also implements **single/double ↔ extended format conversions**. Verified in `tb_fpu` (2+3=5, 5−2=3, 2×3=6, 6/2=3, √4=2, √9=3, single/double 2.5 round-trips) and `tb_cpu_fpu` (the CPU runs a reg-reg FADD with no F-line trap). **Remaining:** wire the conversions into a kernel **memory-operand path** (FMOVE.X/S/D `<ea>`↔FPn — the load/store conversion functions are ready, the kernel EA fetch is not); FCMP/FINT; full IEEE rounding modes + exception flags (datapath is round-toward-zero today). See `docs/FPU_SCOPE.md`. |
