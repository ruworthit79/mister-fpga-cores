...
| 2 | **Integrated FPU** | on-die 68040 FPU | 🟡 | **Foundation landed** (`fpu_040.vhd`, verified by GHDL `tb_fpu`): FP0-FP7 + FPCR/FPSR/FPIAR, FMOVE/FABS/FNEG/FTST, IEEE classification, `present=1`, clean `unimpl`→FPSP path. Remaining: kernel F-line integration, then the HW arithmetic datapath (FADD/FMUL/FDIV/FSQRT…, the big lift — adapt the open MC68881 VHDL core). See `docs/FPU_SCOPE.md`. Interim = LC040 + software FPSP. |
...
| 4 | **RAM ≤ 80 ns, max 256 MB** | 16 SIMM slots, 256 MB max, 80 ns FPM | 🟡 | Emulated RAM lives in the board's DDR3. Config is currently 64/128 MB — add a 256 MB option. "80 ns" is a bus-timing characteristic; the emulated 33 MHz bus models it, backed by faster DDR3. |
