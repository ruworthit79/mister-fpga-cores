...
| 2 | **Integrated FPU** | on-die 68040 FPU | 🟡 | **Foundation + kernel dispatch + HW arithmetic datapath live** (`fpu_040.vhd`): FP0-FP7 + FPCR/FPSR/FPIAR, FMOVE/FABS/FNEG/FTST, IEEE classification, `present=1`, `unimpl`→FPSP. The CPU executes reg-reg F-line ops (verified by GHDL `tb_cpu_fpu`). **New:** an extended-precision (80-bit) **FADD/FSUB/FMUL** arithmetic datapath (special-value handling for NaN/Inf/Zero, normalization), unit-verified in `tb_fpu` (2+3=5, 5−2=3, 2×3=6). **Remaining:** wire these ops into the kernel's F-line decode + the **memory-operand path** (FMOVE.X `<ea>`↔FPn); add FDIV/FSQRT/FCMP/FINT and full IEEE rounding modes/exception flags (currently round-toward-zero). Interim before full integration = LC040 + software FPSP. See `docs/FPU_SCOPE.md`. |
