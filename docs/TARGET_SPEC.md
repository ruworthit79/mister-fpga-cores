...
| 2 | **Integrated FPU** | on-die 68040 FPU | 🟡 | **Foundation + kernel dispatch + HW arithmetic all live and wired** (`fpu_040.vhd`): FP0-FP7 + FPCR/FPSR/FPIAR, FMOVE/FABS/FNEG/FTST, IEEE classification, `present=1`, `unimpl`→FPSP. The CPU **executes the full common 040 FP op set in hardware** — the kernel decodes **FADD ($22) / FSUB ($28) / FMUL ($23) / FDIV ($20) / FSQRT ($04) / FCMP ($38) / FINT ($01)** and drives the 80-bit extended datapath (NaN/Inf/Zero handling, normalization, digit-by-digit sqrt, round-to-integer). The FPU also implements **single/double ↔ extended format conversions**. Verified in `tb_fpu` (2+3=5, 5−2=3, 2×3=6, 6/2=3, √4=2, √9=3, FCMP N/Z, FINT 2.5→2, single/double 2.5 round-trips) and `tb_cpu_fpu` (reg-reg FADD, no trap). Transcendentals (FSIN/FETOX/…) trap to FPSP — as they do on a real 68040. **Area note:** the HW arithmetic datapath is gated by a generic (`cpu_wrapper`/`fpu_040` `HW_ARITH`) — it's **on in simulation** but **off in the Cyclone V bitstream** (it wouldn't fit; the 128-bit divider / 64×64 mult / digit-by-digit sqrt are ~10k combinational nodes), so **hardware runs as an LC040 with the software FPSP** (the authentic interim). Re-enable on a larger FPGA by setting `HW_ARITH => 1`. **Remaining:** kernel **memory-operand path** (conversion functions ready; EA fetch not wired); IEEE rounding modes + exception flags. See `docs/FPU_SCOPE.md`. |
...
| 6 | **832×624 @ 24-bit** | DAFB w/ 2 MB VRAM (needs ~1.56 MB) | ✅ | DAFB now reads out **8 bpp** (indexed via CLUT), **16 bpp** (xRGB1555 direct) and **32 bpp** (xRGB8888 direct), selected by the depth register. Direct-colour scanout verified in `tb_dafb_depth`. Full-screen deep colour at high res still depends on item 5 (VRAM capacity), but the scanout/colour paths are complete. The register interface is currently an internal layout; the hardware-accurate **DAFB II register map** (verified against MAME `dafb.cpp`) is captured in `docs/DAFB_REGISTERS.md` for a ROM-driven register-accurate pass. |
